...
#: Kabuto ships the same code but its data sits at slightly different offsets.
#: Filled in the same way as Kuwagata once someone runs ``navi identify`` on a
#: Kabuto dump; see docs/PIPELINE.md.
KABUTO = Release(
    code="AVIJ",
    title="MEDAROTNVKBT",
    name="Kabuto",
    ...
    script_table_len=370,
    # From the .definelabel table in Normmatt's Medarot-Navi-GBA-Translation
    # (see NOTICE), which targets Kabuto. Unverified against a dump: the
    # build's fingerprint checks still gate every line, so a wrong constant
    # refuses to build rather than corrupting anything.
    font=0x657D60,
    font_kanji=0,
