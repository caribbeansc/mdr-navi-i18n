...
Medarot Navi shipped as Kuwagata and Kabuto on the same day. They are not two
ports: they are one build whose data sits at different offsets, with different
scenes for the cover Medabot. Aligning the two dumps proves it — 2205 stretches
of byte-identical data cover 97.5% of the cartridge, 159 of the 170 event
scripts match to the byte, and the shifts are a handful of small deltas.

So nothing is reversed twice. Every offset in this project is a KUWAGATA
offset, written next to the code that uses it, and `Rom.at` translates one when
it is about to be used on a dump that is not Kuwagata:

```python
rom.write(rom.at(0x658BF0), font_data)   # the dialogue font, in this release
```

`navi/align.py` holds the map — generated by `tools/align.py` from two dumps,
committed as `data/offsets-kabuto.json`, offsets and lengths only, no bytes.
Three rules make it safe:

* **`None` means leave it alone.** An address the map cannot vouch for is one
  where the releases genuinely differ. The caller skips that asset and says so
  in the build report; it never writes there blind.
* **Only constants are translated.** An offset read out of the ROM — a pointer,
  a table entry, an allocated block — is already in this dump's own space, and
  passing it through `at` a second time would land anywhere.
* **Tables of pointers cannot be found by their content**, because their
  content is exactly what differs. Those are located by what they point AT:
  map the first few targets, and search for that sequence. That is how the
  Kabuto script table and battle-message table are placed, and the answer for
  the script table agrees with the one Normmatt's Kabuto-targeted project
  reversed by hand.

A language pack therefore names its lines the way Kuwagata lays them out, in
both releases, and one translation serves both. A line only the other release
has is keyed `str:KBT:645264` so it can never collide, and its translation
lives in `langs/<code>/kabuto/`.
